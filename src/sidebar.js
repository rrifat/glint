...
let state = { conversation: "", tabId: null, windowId: null };
...
top.append(element("span", { className: "eyebrow", text: "GLINT" }));
if (
  browser.sidebarAction ||
  (isPopup ? browser.sidePanel?.open : browser.sidePanel?.close)
) {
  top.append(
    element("button", {
      className: "panel-button",
      text: isPopup ? "Open sidebar" : "Close sidebar",
      on: {
        click: () => {
          try {
...
header.append(
  top,
  element("h1", { text: "Your highlights" }),
  element("p", { className: "intro", text: "Keep what catches your eye." }),
);
...
const currentCount = element("span", { className: "count" });
currentHeading.append(currentCount);
const source = element("p", { className: "website-name" });
const title = element("p", { className: "page-title" });
const empty = element("div", {
  className: "empty",
  text: "Select a passage to keep it here.",
});
empty.append(
  element("small", { text: "Pick a colour, or press Alt + Shift + H." }),
);
...
const libraryBody = element("div");
library.append(librarySummary, libraryBody);
const sites = new Map();
...
const transfer = element("button", {
  className: "show-more",
  text: "Export & transfer",
  on: {
    click: async () => {
      try {
        await browser.tabs.create({
          url: browser.runtime.getURL(
            `transfer.html?conversation=${encodeURIComponent(state.conversation)}`,
          ),
        });
        if (isPopup) window.close();
      } catch (error) {
...
async function refresh() {
  ...
  try {
    ...
    if (state.conversation !== context?.conversation) {
      ...
      current.append(currentList.node);
    }
    state = {
    ...
    source.textContent = website(tab?.url);
    title.textContent =
      context?.title || tab?.title || "Open a web page to get started";
    ...
    if (!context) {
      ...
      empty.textContent = "Select a passage to keep it here.";
      empty.append(
        element("small", { text: "Pick a colour, or press Alt + Shift + H." }),
      );
    }
    ...
    if (version === refreshVersion) showError(error);
  }
}
...
function conversationView(summary) {
  ...
  const name = element("span", { className: "conversation-title" });
  const count = element("span", { className: "count" });
  heading.append(name, count);
  const body = element("div", { className: "conversation-body" });
  ...
  const view = {
    ...
    update(next) {
      record = next;
      name.textContent = next.title || next.url;
      name.title = next.url;
      ...
      try {
        ...
        if (!list) {
          ...
          body.replaceChildren(
            element("p", {
              className: "muted",
              text: "Select a quote to open its page.",
            }),
            list.node,
          );
        }
...
function renderLibrary() {
  ...
  for (const summary of summaries
    .filter((item) => item.conversation !== state.conversation)
  ...
  const valid = new Set(
    [...groups.values()].flat().map((item) => item.conversation),
  );
  for (const id of conversations.keys())
    if (!valid.has(id)) conversations.delete(id);
  ...
  for (const [host, items] of groups) {
    if (!sites.has(host)) {
      ...
      const label = element("span", { className: "site-label" });
      label.append(element("strong", { text: host }));
      const meta = element("small");
      label.append(meta);
      summary.append(badge, label);
      const body = element("div", { className: "website-body" });
    ...
    reconcile(
      ...
      items.map((item) => {
        if (!conversations.has(item.conversation))
          conversations.set(item.conversation, conversationView(item));
        const view = conversations.get(item.conversation);
        view.update(item);
        return view.node;
      }),
    );
  ...
  if (!nodes.length)
    nodes.push(
      element("p", {
        className: "muted",
        text: "Highlights from other conversations will appear here, grouped by website.",
      }),
    );
...
async function refreshLibrary() {
  ...
  try {
    ...
    if (!result?.ok)
      throw new Error(result?.error || "Could not load saved websites.");
    if (version !== libraryVersion || !library.open) return;
    summaries = result.summaries;
    renderLibrary();
    await Promise.all(
      [...conversations.values()]
        .filter((view) => view.node.open)
        .map((view) => view.load()),
    );
  } catch (error) {
    showError(error);
...
async function jump(annotation) {
  try {
    if (annotation.conversation !== state.conversation) {
      await browser.tabs.create({ url: annotation.url });
      return;
    }
    const result = await browser.tabs.sendMessage(state.tabId, {
      type: "scroll",
      id: annotation.id,
    });
    if (!result?.ok)
      showError(
        result?.reason || "Passage is not loaded or its text has changed.",
      );
...
browser.storage.onChanged.addListener((changes, area) => {
  ...
  if (changes[keyFor(state.conversation)]) scheduleRefresh();
    ...
    (changes[LIBRARY_KEY] ||
      [...conversations].some(
        ([id, view]) => view.node.open && changes[keyFor(id)],
      ))
  )
    void refreshLibrary();
});
