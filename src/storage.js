...
export function summarize(conversation, rows) {
  const latest = rows.reduce((a, b) => !a || b.createdAt > a.createdAt ? b : a, null);
  return latest ? { conversation, title: latest.title, url: latest.url, count: rows.length, updatedAt: latest.createdAt } : null;
}
