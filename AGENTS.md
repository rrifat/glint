...
- Provider selectors have not been tested on current signed-in ChatGPT, Claude, Gemini or DeepSeek pages. Stable IDs are used only where exposed. Missing IDs currently share a null-ID candidate bucket, so identical quotations in different messages can restore incorrectly. Improve disambiguation and leave genuinely ambiguous matches unresolved.
